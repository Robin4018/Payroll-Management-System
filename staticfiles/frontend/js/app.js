    ...
    loginForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                const loginModeInput = document.getElementById('login_mode_input'); // We will add this to login.html
                if (loginModeInput) {
                    const currentMode = loginModeInput.value;
                    const userRole = data.role; // 'employee' or 'admin'

                    console.log(`Login Attempt: Mode=${currentMode}, Role=${userRole}`);

                    if (currentMode === 'employee' && userRole !== 'employee') {
                        // User is admin but trying to login on employee tab
                        const errEl = document.getElementById('loginError');
                        errEl.innerText = 'Access Denied: This portal is for Staff members only. Please switch to the Admin tab.';
                        errEl.classList.remove('hidden');
                        return; // S TOP HERE
                    }

                    if (currentMode === 'admin' && userRole === 'employee') {
                        // User is employee but trying to login on admin tab
                        const errEl = document.getElementById('loginError');
                        errEl.innerText = 'Access Denied: This portal is for Administrators only. Please switch to the Employee tab.';
                        errEl.classList.remove('hidden');
                        return; // STOP HERE
                    }
                }
                ...
                if (nextUrl) {
                    ...
                    // Use server-provided redirect (role-based)
...
window.confirm = function (message, title = "Confirm Action", options = {}) {
    ...
    if (typeof options === 'boolean') {
        isDanger = options;
    } else if (options && options.isDanger) {
    ...
    if (isDanger) {
        ...
        yesBtn.style.boxShadow = '0 10px 15px -3px rgba(239, 68, 68, 0.2)';
        ...
        yesBtn.style.backgroundColor = '#2563eb';
        yesBtn.style.boxShadow = '0 10px 15px -3px rgba(37, 99, 235, 0.2)';
        yesBtn.textContent = 'Yes, Proceed';
        iconDiv.innerHTML = '<i class="fas fa-question-circle" style="color: #2563eb;"></i>';
